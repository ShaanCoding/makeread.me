...
new Vue({
  ...
  methods: {
    ...
    getCheckboxes: function (data) {
      ...
      if (data.starsBadge) {
        source +=
          '![Stargazers](https://img.shields.io/github/stars/' +
          data.userName +
          '/' +
          data.repoName +
          '?style=social) ';
    ...
    builtWith: function (data) {
      ...
      if (data.builtWith || data.builtWithList.length > 0) {
        ...
        for (i = 0; i < data.builtWithList.length; i++) {
          source +=
            '* [' +
            data.builtWithList[i].builtWithName +
            '](' +
            data.builtWithList[i].builtWithURL +
            ')\n';
        }
      }
      ...
      if (data.contributing || data.contributingDesc || data.pullRequest) {
        ...
        if (data.contributing) {
          ...
          sourceDesc +=
            ...
            '/' +
            data.repoName +
            '/issues/new) to discuss it, or directly create a pull request after you edit the *README.md* file with necessary changes.\n';
          sourceDesc +=
          ...
          sourceDesc +=
            ...
            '/' +
            data.repoName +
            '/blob/main/CODE_OF_CONDUCT.md) before posting your first idea as well.\n';
        }
    ...
    getAuthors: function (data) {
      ...
      for (i = 0; i < data.authors.length; i++) {
        source +=
          '* **' +
          data.authors[i].authorName +
          '** - *' +
          data.authors[i].authorDesc +
          '* - [' +
          data.authors[i].authorName +
          '](' +
          data.authors[i].authorURL +
          ') - *' +
          data.authors[i].authorContribution +
          '*\n';
      }
    ...
    getAcknowledgements: function (data) {
      ...
      for (i = 0; i < data.acknowledgements.length; i++) {
        source +=
          '* [' +
          data.acknowledgements[i].acknowledgementName +
          '](' +
          data.acknowledgements[i].acknowledgementURL +
          ')\n';
      }
